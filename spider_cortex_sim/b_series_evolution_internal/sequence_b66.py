...
from .requires_sequences_b1_b5 import (
    ...
    require_b65_enteric_assimilation_checkpoint,
)


def _b66_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b66_immune_malaise_attempt(
    variant_name: str,
    *,
    ...
    config = build_b66_immune_malaise_config(
        variant_name,
    ...
    corridor_gate = b66_immune_malaise_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b66_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b66_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("immune_tone_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("malaise_drive_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("recovery_veto_episodes", 0) or 0) * 800.0
...
def run_b66_immune_malaise_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b66_training_episodes: int = 64,
    b66_workers: int = 1,
    b66_search: str = "hybrid",
    b66_ga_population: int = 24,
    b66_ga_generations: int = 8,
    b66_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b66_ga_population, b66_ga_generations, b66_finalists
    b65_report = require_b65_enteric_assimilation_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b65_report["checkpoint"])
    jobs = list(B66_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b66_search in {"fixed", "hybrid"} and int(b66_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b66_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b66_immune_malaise_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b66_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b66_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b66_immune_malaise_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b66_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b66_search in {"ga", "hybrid"}:
        fallback_attempt = run_b66_immune_malaise_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b66_immune_malaise_attempt(
    ...
    summary = {
        ...
        "b65_source": b65_report,
        "b66_search": b66_search,
        "b66_workers": int(b66_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b66_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B66 immune-malaise line and try explicit cytokine release timing"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b66_evolution_summary.json").write_text(
