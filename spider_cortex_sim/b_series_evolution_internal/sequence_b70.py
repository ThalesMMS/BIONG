...
from .config_builders import build_b70_optic_flow_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b69_vestibular_orientation_checkpoint,
)


def _b70_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b70_optic_flow_attempt(
    variant_name: str,
    *,
    ...
    config = build_b70_optic_flow_config(
        variant_name,
    ...
    corridor_gate = b70_optic_flow_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b70_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b70_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("flow_confidence_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("lateral_drift_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("looming_risk_episodes", 0) or 0) * 800.0
...
def run_b70_optic_flow_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b70_training_episodes: int = 64,
    b70_workers: int = 1,
    b70_search: str = "hybrid",
    b70_ga_population: int = 24,
    b70_ga_generations: int = 8,
    b70_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b70_ga_population, b70_ga_generations, b70_finalists
    b69_report = require_b69_vestibular_orientation_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b69_report["checkpoint"])
    jobs = list(B70_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b70_search in {"fixed", "hybrid"} and int(b70_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b70_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b70_optic_flow_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b70_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b70_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b70_optic_flow_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b70_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b70_search in {"ga", "hybrid"}:
        fallback_attempt = run_b70_optic_flow_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b70_optic_flow_attempt(
    ...
    summary = {
        ...
        "b69_source": b69_report,
        "b70_search": b70_search,
        "b70_workers": int(b70_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b70_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B70 optic-flow line and try explicit visual corridor edge gate"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b70_evolution_summary.json").write_text(
