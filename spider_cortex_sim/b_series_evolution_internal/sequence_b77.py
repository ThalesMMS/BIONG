...
from .config_builders import build_b77_olivary_error_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b76_cerebellar_stride_checkpoint,
)


def _b77_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b77_olivary_error_attempt(
    variant_name: str,
    *,
    ...
    config = build_b77_olivary_error_config(
        variant_name,
    ...
    corridor_gate = b77_olivary_error_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b77_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b77_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("prediction_error_episodes", 0) or 0) * 950.0
        + int(corridor_aggregate.get("climbing_fiber_drive_episodes", 0) or 0)
        * 900.0
        + int(corridor_aggregate.get("corrective_timing_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("stability_confidence_episodes", 0) or 0)
        * 800.0
        + int(corridor_aggregate.get("error_lock_or_release_episodes", 0) or 0)
        * 750.0
...
def run_b77_olivary_error_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b77_training_episodes: int = 64,
    b77_workers: int = 1,
    b77_search: str = "hybrid",
    b77_ga_population: int = 24,
    b77_ga_generations: int = 8,
    b77_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b77_ga_population, b77_ga_generations, b77_finalists
    b76_report = require_b76_cerebellar_stride_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b76_report["checkpoint"])
    jobs = list(B77_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b77_search in {"fixed", "hybrid"} and int(b77_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b77_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b77_olivary_error_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b77_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b77_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b77_olivary_error_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b77_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b77_search in {"ga", "hybrid"}:
        fallback_attempt = run_b77_olivary_error_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b77_olivary_error_attempt(
    ...
    summary = {
        ...
        "b76_source": b76_report,
        "b77_search": b77_search,
        "b77_workers": int(b77_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b77_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B77 olivary line and try vestibulo-cerebellar correction"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b77_evolution_summary.json").write_text(
