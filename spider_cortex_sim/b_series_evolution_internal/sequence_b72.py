...
from .config_builders import build_b72_pulvinar_attention_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b71_tectal_orienting_checkpoint,
)


def _b72_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b72_pulvinar_attention_attempt(
    variant_name: str,
    *,
    ...
    config = build_b72_pulvinar_attention_config(
        variant_name,
    ...
    corridor_gate = b72_pulvinar_attention_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b72_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b72_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("focus_signal_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("distractor_load_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("filter_gain_episodes", 0) or 0) * 800.0
        + int(corridor_aggregate.get("attention_lock_or_release_episodes", 0) or 0)
        * 700.0
...
def run_b72_pulvinar_attention_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b72_training_episodes: int = 64,
    b72_workers: int = 1,
    b72_search: str = "hybrid",
    b72_ga_population: int = 24,
    b72_ga_generations: int = 8,
    b72_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b72_ga_population, b72_ga_generations, b72_finalists
    b71_report = require_b71_tectal_orienting_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b71_report["checkpoint"])
    jobs = list(B72_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b72_search in {"fixed", "hybrid"} and int(b72_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b72_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b72_pulvinar_attention_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b72_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b72_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b72_pulvinar_attention_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b72_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b72_search in {"ga", "hybrid"}:
        fallback_attempt = run_b72_pulvinar_attention_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b72_pulvinar_attention_attempt(
    ...
    summary = {
        ...
        "b71_source": b71_report,
        "b72_search": b72_search,
        "b72_workers": int(b72_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b72_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B72 pulvinar line and try explicit reticular distractor gate"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b72_evolution_summary.json").write_text(
