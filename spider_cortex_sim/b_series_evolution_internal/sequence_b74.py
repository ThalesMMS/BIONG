...
from .config_builders import build_b74_thalamic_rebound_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b73_reticular_inhibition_checkpoint,
)


def _b74_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b74_thalamic_rebound_attempt(
    variant_name: str,
    *,
    ...
    config = build_b74_thalamic_rebound_config(
        variant_name,
    ...
    corridor_gate = b74_thalamic_rebound_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b74_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b74_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("rebound_potential_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("inhibition_aftereffect_episodes", 0) or 0)
        * 850.0
        + int(corridor_aggregate.get("release_window_episodes", 0) or 0) * 800.0
        + int(corridor_aggregate.get("rebound_lock_or_release_episodes", 0) or 0)
        * 700.0
...
def run_b74_thalamic_rebound_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b74_training_episodes: int = 64,
    b74_workers: int = 1,
    b74_search: str = "hybrid",
    b74_ga_population: int = 24,
    b74_ga_generations: int = 8,
    b74_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b74_ga_population, b74_ga_generations, b74_finalists
    b73_report = require_b73_reticular_inhibition_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b73_report["checkpoint"])
    jobs = list(B74_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b74_search in {"fixed", "hybrid"} and int(b74_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b74_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b74_thalamic_rebound_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b74_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b74_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b74_thalamic_rebound_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b74_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b74_search in {"ga", "hybrid"}:
        fallback_attempt = run_b74_thalamic_rebound_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b74_thalamic_rebound_attempt(
    ...
    summary = {
        ...
        "b73_source": b73_report,
        "b74_search": b74_search,
        "b74_workers": int(b74_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b74_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B74 rebound line and try basal-thalamic release timing"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b74_evolution_summary.json").write_text(
