...
from .requires_sequences_b1_b5 import (
    ...
    require_b63_periaqueductal_escape_checkpoint,
)


def _b64_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b64_vagal_recovery_attempt(
    variant_name: str,
    *,
    ...
    config = build_b64_vagal_recovery_config(
        variant_name,
    ...
    corridor_gate = b64_vagal_recovery_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b64_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b64_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("recovery_tone_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("vagal_brake_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("post_escape_bias_episodes", 0) or 0) * 800.0
        + int(corridor_aggregate.get("lock_or_hold_episodes", 0) or 0) * 700.0
...
def run_b64_vagal_recovery_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b64_training_episodes: int = 64,
    b64_workers: int = 1,
    b64_search: str = "hybrid",
    b64_ga_population: int = 24,
    b64_ga_generations: int = 8,
    b64_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b64_ga_population, b64_ga_generations, b64_finalists
    b63_report = require_b63_periaqueductal_escape_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b63_report["checkpoint"])
    jobs = list(B64_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b64_search in {"fixed", "hybrid"} and int(b64_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b64_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b64_vagal_recovery_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b64_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b64_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b64_vagal_recovery_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b64_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b64_search in {"ga", "hybrid"}:
        fallback_attempt = run_b64_vagal_recovery_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b64_vagal_recovery_attempt(
    ...
    summary = {
        ...
        "b63_source": b63_report,
        "b64_search": b64_search,
        "b64_workers": int(b64_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b64_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B64 recovery-brake line and try explicit shelter digestion phase"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b64_evolution_summary.json").write_text(
