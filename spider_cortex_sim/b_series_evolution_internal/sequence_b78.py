...
from .config_builders import build_b78_vestibular_balance_config
...
from .gates_b77_requires import b78_vestibular_balance_corridor_gate_result

from .requires_sequences_b1_b5 import (
    _run_episode_payload,
    require_b77_olivary_error_checkpoint,
)


def _b78_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b78_vestibular_balance_attempt(
    variant_name: str,
    *,
    ...
    config = build_b78_vestibular_balance_config(
        variant_name,
    ...
    corridor_gate = b78_vestibular_balance_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b78_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b78_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("balance_error_episodes", 0) or 0) * 950.0
        + int(corridor_aggregate.get("head_stabilization_episodes", 0) or 0)
        * 900.0
        + int(corridor_aggregate.get("locomotor_confidence_episodes", 0) or 0)
        * 850.0
        + int(corridor_aggregate.get("slip_risk_episodes", 0) or 0) * 800.0
        + int(corridor_aggregate.get("balance_lock_or_release_episodes", 0) or 0)
        * 750.0
...
def run_b78_vestibular_balance_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b78_training_episodes: int = 64,
    b78_workers: int = 1,
    b78_search: str = "hybrid",
    b78_ga_population: int = 24,
    b78_ga_generations: int = 8,
    b78_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b78_ga_population, b78_ga_generations, b78_finalists
    b77_report = require_b77_olivary_error_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b77_report["checkpoint"])
    jobs = list(B78_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b78_search in {"fixed", "hybrid"} and int(b78_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b78_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b78_vestibular_balance_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b78_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b78_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b78_vestibular_balance_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b78_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b78_search in {"ga", "hybrid"}:
        fallback_attempt = run_b78_vestibular_balance_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b78_vestibular_balance_attempt(
    ...
    summary = {
        ...
        "b77_source": b77_report,
        "b78_search": b78_search,
        "b78_workers": int(b78_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b78_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B78 vestibular line and try proprioceptive balance correction"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b78_evolution_summary.json").write_text(
