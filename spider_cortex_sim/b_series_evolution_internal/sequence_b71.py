...
from .config_builders import build_b71_tectal_orienting_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b70_optic_flow_checkpoint,
)


def _b71_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b71_tectal_orienting_attempt(
    variant_name: str,
    *,
    ...
    config = build_b71_tectal_orienting_config(
        variant_name,
    ...
    corridor_gate = b71_tectal_orienting_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b71_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b71_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("target_salience_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("orienting_gain_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("collision_veto_episodes", 0) or 0) * 800.0
...
def run_b71_tectal_orienting_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b71_training_episodes: int = 64,
    b71_workers: int = 1,
    b71_search: str = "hybrid",
    b71_ga_population: int = 24,
    b71_ga_generations: int = 8,
    b71_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b71_ga_population, b71_ga_generations, b71_finalists
    b70_report = require_b70_optic_flow_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b70_report["checkpoint"])
    jobs = list(B71_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b71_search in {"fixed", "hybrid"} and int(b71_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b71_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b71_tectal_orienting_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b71_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b71_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b71_tectal_orienting_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b71_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b71_search in {"ga", "hybrid"}:
        fallback_attempt = run_b71_tectal_orienting_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b71_tectal_orienting_attempt(
    ...
    summary = {
        ...
        "b70_source": b70_report,
        "b71_search": b71_search,
        "b71_workers": int(b71_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b71_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B71 tectal line and try explicit superior-colliculus threat map"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b71_evolution_summary.json").write_text(
