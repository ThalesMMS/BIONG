...
from .config_builders import build_b68_motor_pacing_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b67_glial_energy_checkpoint,
)


def _b68_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b68_motor_pacing_attempt(
    variant_name: str,
    *,
    ...
    config = build_b68_motor_pacing_config(
        variant_name,
    ...
    corridor_gate = b68_motor_pacing_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b68_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b68_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("motor_reserve_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("stride_pacing_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("overexertion_risk_episodes", 0) or 0) * 800.0
...
def run_b68_motor_pacing_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b68_training_episodes: int = 64,
    b68_workers: int = 1,
    b68_search: str = "hybrid",
    b68_ga_population: int = 24,
    b68_ga_generations: int = 8,
    b68_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b68_ga_population, b68_ga_generations, b68_finalists
    b67_report = require_b67_glial_energy_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b67_report["checkpoint"])
    jobs = list(B68_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b68_search in {"fixed", "hybrid"} and int(b68_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b68_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b68_motor_pacing_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b68_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b68_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b68_motor_pacing_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b68_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b68_search in {"ga", "hybrid"}:
        fallback_attempt = run_b68_motor_pacing_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b68_motor_pacing_attempt(
    ...
    summary = {
        ...
        "b67_source": b67_report,
        "b68_search": b68_search,
        "b68_workers": int(b68_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b68_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B68 motor-pacing line and try explicit stride fatigue rebound"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b68_evolution_summary.json").write_text(
