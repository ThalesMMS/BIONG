...
from .config_builders import build_b69_vestibular_orientation_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b68_motor_pacing_checkpoint,
)


def _b69_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b69_vestibular_orientation_attempt(
    variant_name: str,
    *,
    ...
    config = build_b69_vestibular_orientation_config(
        variant_name,
    ...
    corridor_gate = b69_vestibular_orientation_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b69_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b69_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("heading_confidence_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("turn_error_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("orientation_stability_episodes", 0) or 0) * 800.0
        + int(corridor_aggregate.get("lock_or_release_episodes", 0) or 0) * 700.0
...
def run_b69_vestibular_orientation_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b69_training_episodes: int = 64,
    b69_workers: int = 1,
    b69_search: str = "hybrid",
    b69_ga_population: int = 24,
    b69_ga_generations: int = 8,
    b69_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b69_ga_population, b69_ga_generations, b69_finalists
    b68_report = require_b68_motor_pacing_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b68_report["checkpoint"])
    jobs = list(B69_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b69_search in {"fixed", "hybrid"} and int(b69_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b69_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b69_vestibular_orientation_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b69_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b69_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b69_vestibular_orientation_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b69_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b69_search in {"ga", "hybrid"}:
        fallback_attempt = run_b69_vestibular_orientation_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b69_vestibular_orientation_attempt(
    ...
    summary = {
        ...
        "b68_source": b68_report,
        "b69_search": b69_search,
        "b69_workers": int(b69_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b69_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B69 vestibular line and try explicit optic-flow heading gate"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b69_evolution_summary.json").write_text(
