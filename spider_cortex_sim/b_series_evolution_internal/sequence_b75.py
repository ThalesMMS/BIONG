...
from .config_builders import build_b75_basal_thalamic_release_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b74_thalamic_rebound_checkpoint,
)


def _b75_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b75_basal_thalamic_release_attempt(
    variant_name: str,
    *,
    ...
    config = build_b75_basal_thalamic_release_config(
        variant_name,
    ...
    corridor_gate = b75_basal_thalamic_release_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b75_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b75_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("release_timing_drive_episodes", 0) or 0)
        * 900.0
        + int(corridor_aggregate.get("go_disinhibition_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("nogo_brake_episodes", 0) or 0) * 800.0
        + int(corridor_aggregate.get("burst_window_episodes", 0) or 0) * 750.0
        + int(corridor_aggregate.get("release_lock_or_stride_episodes", 0) or 0)
        * 700.0
...
def run_b75_basal_thalamic_release_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b75_training_episodes: int = 64,
    b75_workers: int = 1,
    b75_search: str = "hybrid",
    b75_ga_population: int = 24,
    b75_ga_generations: int = 8,
    b75_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b75_ga_population, b75_ga_generations, b75_finalists
    b74_report = require_b74_thalamic_rebound_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b74_report["checkpoint"])
    jobs = list(B75_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b75_search in {"fixed", "hybrid"} and int(b75_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b75_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b75_basal_thalamic_release_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b75_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b75_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b75_basal_thalamic_release_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b75_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b75_search in {"ga", "hybrid"}:
        fallback_attempt = run_b75_basal_thalamic_release_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b75_basal_thalamic_release_attempt(
    ...
    summary = {
        ...
        "b74_source": b74_report,
        "b75_search": b75_search,
        "b75_workers": int(b75_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b75_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B75 release timing and try cerebellar stride gating"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b75_evolution_summary.json").write_text(
