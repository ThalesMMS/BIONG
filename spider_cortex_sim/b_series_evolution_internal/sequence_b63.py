...
from .requires_sequences_b1_b5 import (
    ...
    require_b62_defensive_mode_selector_checkpoint,
)


def _b63_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b63_periaqueductal_escape_attempt(
    variant_name: str,
    *,
    ...
    config = build_b63_periaqueductal_escape_config(
        variant_name,
    ...
    corridor_gate = b63_periaqueductal_escape_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b63_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b63_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("escape_phase_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("escape_pressure_episodes", 0) or 0) * 800.0
        + int(corridor_aggregate.get("shelter_vector_episodes", 0) or 0) * 750.0
...
def run_b63_periaqueductal_escape_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b63_training_episodes: int = 64,
    b63_workers: int = 1,
    b63_search: str = "hybrid",
    b63_ga_population: int = 24,
    b63_ga_generations: int = 8,
    b63_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b63_ga_population, b63_ga_generations, b63_finalists
    b62_report = require_b62_defensive_mode_selector_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b62_report["checkpoint"])
    jobs = list(B63_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b63_search in {"fixed", "hybrid"} and int(b63_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b63_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b63_periaqueductal_escape_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b63_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b63_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b63_periaqueductal_escape_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b63_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b63_search in {"ga", "hybrid"}:
        fallback_attempt = run_b63_periaqueductal_escape_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b63_periaqueductal_escape_attempt(
    ...
    summary = {
        ...
        "b62_source": b62_report,
        "b63_search": b63_search,
        "b63_workers": int(b63_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b63_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B63 escape-sequence line and try explicit post-shelter recovery"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b63_evolution_summary.json").write_text(
