...
from .requires_sequences_b1_b5 import (
    ...
    require_b66_immune_malaise_checkpoint,
)


def _b67_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b67_glial_energy_attempt(
    variant_name: str,
    *,
    ...
    config = build_b67_glial_energy_config(
        variant_name,
    ...
    corridor_gate = b67_glial_energy_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b67_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b67_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("glial_reserve_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("lactate_support_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("fatigue_pressure_episodes", 0) or 0) * 800.0
...
def run_b67_glial_energy_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b67_training_episodes: int = 64,
    b67_workers: int = 1,
    b67_search: str = "hybrid",
    b67_ga_population: int = 24,
    b67_ga_generations: int = 8,
    b67_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b67_ga_population, b67_ga_generations, b67_finalists
    b66_report = require_b66_immune_malaise_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b66_report["checkpoint"])
    jobs = list(B67_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b67_search in {"fixed", "hybrid"} and int(b67_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b67_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b67_glial_energy_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b67_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b67_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b67_glial_energy_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b67_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b67_search in {"ga", "hybrid"}:
        fallback_attempt = run_b67_glial_energy_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b67_glial_energy_attempt(
    ...
    summary = {
        ...
        "b66_source": b66_report,
        "b67_search": b67_search,
        "b67_workers": int(b67_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b67_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B67 glial-energy line and try explicit fatigue rebound timing"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b67_evolution_summary.json").write_text(
