...
from .config_builders import build_b76_cerebellar_stride_config
...
from .requires_sequences_b1_b5 import (
    ...
    require_b75_basal_thalamic_release_checkpoint,
)


def _b76_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b76_cerebellar_stride_attempt(
    variant_name: str,
    *,
    ...
    config = build_b76_cerebellar_stride_config(
        variant_name,
    ...
    corridor_gate = b76_cerebellar_stride_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b76_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b76_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("stride_timing_signal_episodes", 0) or 0)
        * 900.0
        + int(corridor_aggregate.get("error_correction_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("burst_smoothing_episodes", 0) or 0) * 800.0
        + int(corridor_aggregate.get("stride_gate_episodes", 0) or 0) * 750.0
        + int(corridor_aggregate.get("stride_lock_or_release_episodes", 0) or 0)
        * 700.0
...
def run_b76_cerebellar_stride_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b76_training_episodes: int = 64,
    b76_workers: int = 1,
    b76_search: str = "hybrid",
    b76_ga_population: int = 24,
    b76_ga_generations: int = 8,
    b76_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b76_ga_population, b76_ga_generations, b76_finalists
    b75_report = require_b75_basal_thalamic_release_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b75_report["checkpoint"])
    jobs = list(B76_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b76_search in {"fixed", "hybrid"} and int(b76_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b76_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b76_cerebellar_stride_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b76_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b76_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b76_cerebellar_stride_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b76_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b76_search in {"ga", "hybrid"}:
        fallback_attempt = run_b76_cerebellar_stride_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b76_cerebellar_stride_attempt(
    ...
    summary = {
        ...
        "b75_source": b75_report,
        "b76_search": b76_search,
        "b76_workers": int(b76_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b76_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B76 stride gate and try vestibulo-cerebellar correction"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b76_evolution_summary.json").write_text(
