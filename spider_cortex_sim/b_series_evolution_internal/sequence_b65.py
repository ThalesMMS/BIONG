...
from .requires_sequences_b1_b5 import (
    ...
    require_b64_vagal_recovery_checkpoint,
)


def _b65_attempt_dir(
    root: str | Path,
    variant_name: str,
    seed: int,
...
def run_b65_enteric_assimilation_attempt(
    variant_name: str,
    *,
    ...
    config = build_b65_enteric_assimilation_config(
        variant_name,
    ...
    corridor_gate = b65_enteric_assimilation_corridor_gate_result(corridor_results)
    accepted = bool(
        easy_gate["passed"]
    ...
    attempt_dir = _b65_attempt_dir(root, variant_name, seed, candidate_id)
...
def _b65_attempt_fitness(attempt: dict[str, object]) -> float:
    ...
    score = (
        ...
        + int(corridor_aggregate.get("enteric_tone_episodes", 0) or 0) * 900.0
        + int(corridor_aggregate.get("assimilation_drive_episodes", 0) or 0) * 850.0
        + int(corridor_aggregate.get("forage_readiness_episodes", 0) or 0) * 800.0
...
def run_b65_enteric_assimilation_sequence(
    *,
    root: str | Path = B_SERIES_EVOLUTION_ROOT,
    seed: int = 7,
    b65_training_episodes: int = 64,
    b65_workers: int = 1,
    b65_search: str = "hybrid",
    b65_ga_population: int = 24,
    b65_ga_generations: int = 8,
    b65_finalists: int = 6,
    reward_profile: str = "ecological",
    operational_profile: str = "default_v1",
    noise_profile: str = "none",
) -> dict[str, object]:
    del b65_ga_population, b65_ga_generations, b65_finalists
    b64_report = require_b64_vagal_recovery_checkpoint(root=root, seed=seed)
    source_checkpoint = str(b64_report["checkpoint"])
    jobs = list(B65_FIXED_EVOLUTION_ATTEMPTS)
    attempts_by_name: dict[str, dict[str, object]] = {}
    if b65_search in {"fixed", "hybrid"} and int(b65_workers) > 1 and len(jobs) > 1:
        with ProcessPoolExecutor(max_workers=min(int(b65_workers), len(jobs))) as pool:
            future_map = {
                pool.submit(
                    run_b65_enteric_assimilation_attempt,
                    variant_name,
                    source_checkpoint=source_checkpoint,
                    root=root,
                    seed=seed,
                    training_episodes=b65_training_episodes,
                    reward_profile=reward_profile,
                    operational_profile=operational_profile,
                    noise_profile=noise_profile,
    ...
    if b65_search in {"fixed", "hybrid"} and len(attempts_by_name) < len(jobs):
        for variant_name in jobs:
            if variant_name in attempts_by_name:
                continue
            attempt = run_b65_enteric_assimilation_attempt(
                variant_name,
                source_checkpoint=source_checkpoint,
                root=root,
                seed=seed,
            ...
            if b65_search == "hybrid" and attempt.get("status") == "accepted":
                break
    ...
    if selected is None and b65_search in {"ga", "hybrid"}:
        fallback_attempt = run_b65_enteric_assimilation_attempt(
    ...
    if selected is not None:
        promoted_attempt = run_b65_enteric_assimilation_attempt(
    ...
    summary = {
        ...
        "b64_source": b64_report,
        "b65_search": b65_search,
        "b65_workers": int(b65_workers),
        "attempts": attempts,
        "fixed_attempts": fixed_attempts,
        "fallback_attempt": fallback_attempt,
        "best_attempt": (
            max(attempts, key=_b65_attempt_fitness) if attempts else None
        ),
        "next_recommendation": (
            None
            if accepted_variant is not None
            else "discard B65 enteric line and try explicit metabolic release timing"
        ),
    }
    root_path = Path(root)
    root_path.mkdir(parents=True, exist_ok=True)
    (root_path / "b65_evolution_summary.json").write_text(
